...
class TrainConfig:
    seed: int = 0
    epochs: int = 2000          # CatBoost uses iterations
    lr: float = 0.03
    depth: int = 6
...
def load_labels(data_dir: str) -> pd.Series:
    ...
    if LABEL_COL not in df.columns:
        raise ValueError(f"Expected '{LABEL_COL}' in metadata.csv.")
...
def align_XY(X: pd.DataFrame, Y: pd.DataFrame, lab: pd.Series) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series]:
    ...
    if len(common) < 50:
        raise ValueError(f"Too few aligned samples ({len(common)}).")
...
def fit_catboost_multi(X_tr: np.ndarray, Y_tr: np.ndarray,
                       X_va: np.ndarray, Y_va: np.ndarray,
                       cfg: TrainConfig) -> List[CatBoostRegressor]:
    models = []
    for j in range(Y_tr.shape[1]):
        m = CatBoostRegressor(
            ...
            random_seed=cfg.seed + j,
            od_type="Iter",
            od_wait=cfg.early_stopping_rounds,
            verbose=cfg.verbose
        )
        m.fit(X_tr, Y_tr[:, j], eval_set=(X_va, Y_va[:, j]), use_best_model=True)
        models.append(m)
    return models


def predict_catboost_multi(models: List[CatBoostRegressor], X: np.ndarray) -> np.ndarray:
    preds = []
    for m in models:
...
def run_levelA(data_dir: str, dataset: str, cfg: TrainConfig, rus_repeats: int):
    ...
    print(f"[INFO] Model: CatBoost (12x RMSE)")
    ...
    for fold, (tr, va) in enumerate(skf.split(X_train_all, lab_train_all), start=1):
        ...
        for rep in range(rus_repeats):
            ...
            # internal val for early stopping (within RUS)
            ...
            models = fit_catboost_multi(X_tr2, Y_tr2, X_va2, Y_va2, cfg)

            Y_tr_pred = predict_catboost_multi(models, X_tr)
            Y_va_pred = predict_catboost_multi(models, X_va)

            rep_mse.append(mean_squared_error(Y_va, Y_va_pred))
            rep_auc.append(score_layer_auc(lab_tr, Y_tr_pred, lab_va, Y_va_pred, seed=cfg.seed + 1000*fold + rep))
    ...
    # final training: average preds across repeats
    ...
    for rep in range(rus_repeats):
        ...
        Y_train_pred_all.append(predict_catboost_multi(models, X_train_all))
        Y_held_pred_all.append(predict_catboost_multi(models, X_held))
...
def main():
    ...
    run_levelA(args.data_dir, args.dataset, cfg, rus_repeats=args.rus_repeats)
